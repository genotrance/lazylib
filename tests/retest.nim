...
## `PCRE (Perl-Compatible Regular Expressions) <http://www.pcretest.org>`_
...
when defined(gcDestructors):
  proc `=destroy`(x: var RegexDesc) =
    pcretest.free_substring(cast[cstring](x.h))
    if not isNil(x.e):
      pcretest.free_study(x.e)
...
proc rawCompile(pattern: string, flags: cint): ptr Pcre =
  var
    msg: cstring = ""
    offset: cint = 0
  result = pcretest.compile(pattern, flags, addr(msg), addr(offset), nil)
  if result == nil:
...
proc re*(s: string, flags = {reStudy}): Regex =
  ...
  if reStudy in flags:
    ...
    var options: cint = 0
    var hasJit: cint = 0
    if pcretest.config(pcretest.CONFIG_JIT, addr hasJit) == 0:
      if hasJit == 1'i32:
        options = pcretest.STUDY_JIT_COMPILE
    result.e = pcretest.study(result.h, options, addr msg)
...
proc matchOrFind(buf: cstring, pattern: Regex, matches: var openArray[string],
  ...
  var
    ...
    res = pcretest.exec(pattern.h, pattern.e, buf, bufSize, start, flags,
...
proc findBounds*(buf: cstring, pattern: Regex, matches: var openArray[string],
  ...
  var
    rtarray = initRtArray[cint]((matches.len+1)*3)
    rawMatches = rtarray.getRawData
    res = pcretest.exec(pattern.h, pattern.e, buf, bufSize.cint, start.cint, 0'i32,
      cast[ptr cint](rawMatches), (matches.len+1).cint*3)
...
proc matchOrFind(buf: cstring, pattern: Regex, start, bufSize: int, flags: cint): cint =
  var
    rtarray = initRtArray[cint](3)
    rawMatches = rtarray.getRawData
  result = pcretest.exec(pattern.h, pattern.e, buf, bufSize.cint, start.cint, flags,
                    cast[ptr cint](rawMatches), 3)
  if result >= 0'i32:
...
proc matchLen*(s: string, pattern: Regex, matches: var openArray[string],
  ...
  ## of zero can happen.
  result = matchOrFind(cstring(s), pattern, matches, start.cint, s.len.cint, pcretest.ANCHORED)
...
proc matchLen*(buf: cstring, pattern: Regex, matches: var openArray[string],
  ...
  ## if there is no match, ``-1`` is returned. Note that a match length
  ## of zero can happen.
  return matchOrFind(buf, pattern, matches, start.cint, bufSize.cint, pcretest.ANCHORED)
...
proc matchLen*(s: string, pattern: Regex, start = 0): int {.inline.} =
  ...
  result = matchOrFind(cstring(s), pattern, start.cint, s.len.cint, pcretest.ANCHORED)

proc matchLen*(buf: cstring, pattern: Regex, start = 0, bufSize: int): int {.inline.} =
  ## the same as ``match``, but it returns the length of the match,
  ## if there is no match, ``-1`` is returned. Note that a match length
  ## of zero can happen.
  result = matchOrFind(buf, pattern, start.cint, bufSize, pcretest.ANCHORED)
...
iterator findAll*(s: string, pattern: Regex, start = 0): string =
  ...
  var
    ...
    rawMatches = rtarray.getRawData
  while true:
    let res = pcretest.exec(pattern.h, pattern.e, s, len(s).cint, i, 0'i32,
      cast[ptr cint](rawMatches), 3)
    if res < 0'i32: break
    let a = rawMatches[0]
    let b = rawMatches[1]
...
iterator findAll*(buf: cstring, pattern: Regex, start = 0, bufSize: int): string =
  ...
  var
    ...
    rawMatches = rtarray.getRawData
  while true:
    let res = pcretest.exec(pattern.h, pattern.e, buf, bufSize.cint, i, 0'i32,
      cast[ptr cint](rawMatches), 3)
    if res < 0'i32: break
    let a = rawMatches[0]
    let b = rawMatches[1]
